        ...
        <div className={`${showBanner ? 'pt-[134px] md:pt-[164px]' : 'pt-[94px] md:pt-[104px]'}`}>
            ...
            <nav className={`mx-4 md:mx-14 lg:px-14 lg:mx-auto lg:max-w-[1440px] transition-all duration-300 ease-in-out fixed top-0 left-0 right-0 z-[99] ${showBanner ? 'mt-[50px] md:mt-[80px]' : 'mt-[10px] md:mt-[20px]' }`}>
                <div className={`flex flex-col gap-[10px] rounded-[8px] border border-[#E6E6E6] bg-white  transition-all duration-150 ease-in-out`}>
                    <div className={`flex items-center justify-between px-4 rounded-[8px]`}>
                        ...
                        <div className="hidden lg:flex items-center justify-center h-full">
                            {
                                navLinks.map((link, index) => (
                                    <a href={link.href} className={`transition-all duration-150 ease-in-out text-[16px]/[16px] font-normal tracking-[-0.96px] hover:text-[#169B4C] capitalize ${path.toLowerCase() === link.href.toLowerCase() ? 'text-[#169B4C]' : 'text-[#999999]'} h-full px-2 xl:px-4`} key={index}>
                                        {link.name}
                                    </a>
                                ))
                            }
                            {
                                menu.map((menuItem, index) => (
                                    <NavDrop title={menuItem.title} menu={menuItem.menu} key={index} />
                                ))
                            }
                        </div>
                        {/* login container */}
                        <div className="hidden lg:flex items-center justify-center gap-4">
                            <a href="http://test.l4lh6qmixi-ez94do5je6mr.p.temp-site.link/login/" className="text-[#000000] text-[16px]/[16px] font-medium tracking-[-0.96px] capitalize hover:text-[#169B4C] transition-all duration-150 ease-in-out">login</a>
